...
package App::GUI::Chaosgraph::Frame;
use base qw/Wx::Frame/;
use App::GUI::Chaosgraph::Frame::Part::ColorFlow;
use App::GUI::Chaosgraph::Frame::Part::ColorBrowser;
use App::GUI::Chaosgraph::Frame::Part::ColorPicker;
use App::GUI::Chaosgraph::Frame::Part::PenLine;
use App::GUI::Chaosgraph::Frame::Part::Board;
use App::GUI::Chaosgraph::Dialog::About;
use App::GUI::Chaosgraph::ProgressBar;
use App::GUI::Chaosgraph::Settings;
use App::GUI::Chaosgraph::Config;
...
sub new {
    ...
    $self->{'config'} = App::GUI::Chaosgraph::Config->new();
    ...
    $self->{'color'}{'start'}   = App::GUI::Chaosgraph::Frame::Part::ColorBrowser->new( $self->{'tab'}{'pen'}, 'start', { red => 20, green => 20, blue => 110 } );
    $self->{'color'}{'end'}     = App::GUI::Chaosgraph::Frame::Part::ColorBrowser->new( $self->{'tab'}{'pen'}, 'end',  { red => 110, green => 20, blue => 20 } );

    $self->{'color'}{'startio'} = App::GUI::Chaosgraph::Frame::Part::ColorPicker->new( $self->{'tab'}{'pen'}, $self, 'Start Color IO', $self->{'config'}->get_value('color') , 162, 1);
    $self->{'color'}{'endio'}   = App::GUI::Chaosgraph::Frame::Part::ColorPicker->new( $self->{'tab'}{'pen'}, $self, 'End Color IO', $self->{'config'}->get_value('color') , 162, 7);

    $self->{'color_flow'}       = App::GUI::Chaosgraph::Frame::Part::ColorFlow->new( $self->{'tab'}{'pen'}, $self );
    $self->{'line'}             = App::GUI::Chaosgraph::Frame::Part::PenLine->new( $self->{'tab'}{'pen'} );

    $self->{'progress'}            = App::GUI::Chaosgraph::ProgressBar->new( $self, 450, 5, { red => 20, green => 20, blue => 110 });
    $self->{'board'}               = App::GUI::Chaosgraph::Frame::Part::Board->new( $self , 600, 600 );
    $self->{'dialog'}{'about'}     = App::GUI::Chaosgraph::Dialog::About->new();
    ...
    Wx::Event::EVT_BUTTON( $self, $self->{'btn'}{'write_next'},  sub {
        ...
        $self->inc_base_counter unless App::GUI::Chaosgraph::Settings::are_equal( $self->{'last_file_settings'}, $data );
...
sub open_settings_dialog {
    ...
    if (not ref $ret) { $self->SetStatusText( $ret, 0) }
        ...
        my $dir = App::GUI::Chaosgraph::Settings::extract_dir( $path );
...
sub save_image_dialog {
    ...
    else     { $self->{'config'}->set_value('save_dir', App::GUI::Chaosgraph::Settings::extract_dir( $path )) }
}
...
sub inc_base_counter {
    ...
    $dir = App::GUI::Chaosgraph::Settings::expand_path( $dir );
...
sub change_base_dir {
    ...
    $new_dir = App::GUI::Chaosgraph::Settings::shrink_path( $new_dir ) . '/';
...
sub open_setting_file {
    my ($self, $file ) = @_;
    my $data = App::GUI::Chaosgraph::Settings::load( $file );
    if (ref $data) {
        $self->set_data( $data );
        $self->draw;
        my $dir = App::GUI::Chaosgraph::Settings::extract_dir( $file );
...
sub write_settings_file {
    my ($self, $file)  = @_;
    my $ret = App::GUI::Chaosgraph::Settings::write( $file, $self->get_data );
    if ($ret){ $self->SetStatusText( $ret, 0 ) }
    else     {
        $self->{'config'}->add_setting_file( $file );
        $self->update_recent_settings_menu();
...
sub write_image {
    ...
    $file = App::GUI::Chaosgraph::Settings::shrink_path( $file );
